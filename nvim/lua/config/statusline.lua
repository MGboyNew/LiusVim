-- https://github.com/kyazdani42/dotfiles/blob/d9a9035c6bea5b045a1f15f4055d8f84bffa157b/config/common/nvim/lua/statusline.lua
-- 源于上面 我是直接拿来用的,他写了几个au监听了事件 从而实现隐藏了文件树中的statusline
local api = vim.api
...
local func = [[
	function! Status()
		return luaeval("require 'config/statusline'.update()")
	endfunction

	augroup StatusLine
	au!
	au TabEnter,WinLeave,BufEnter,WinEnter,VimEnter * lua require'config/statusline'.clear()
	au BufEnter,VimEnter * setlocal statusline=%!Status()
	au DirChanged * lua require'config/statusline'.update_git()
	augroup END
]]
api.nvim_exec(func, '')
